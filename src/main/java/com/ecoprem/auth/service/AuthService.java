...
public class AuthService {
    ...
    public LoginResult login(LoginRequest request, HttpServletRequest servletRequest) {
        ...
        try {
            ...
            user = userRepository.findByEmailWithStatusAndRoles(request.getEmail())
            ...
            // 👇 Agora o token será gerado fora, via LoginFinalizerService
            return new LoginResult(null, user);

        } finally {
            if (user != null) {
                recordLoginAttempt(user, ipAddress, userAgent, authenticated, failureReason);
            }
        }
    }
